...
create table "user" ("id" serial primary key, "username" varchar(255) null, "password_hash" varchar(255) null, "first_name" varchar(255) null, "last_name" varchar(255) null, "mobile" varchar(255) null, "email" varchar(255) null, "registered_at" timestamptz(0) null, "last_login" timestamptz(0) null, "address" varchar(255) null, "role" text check ("role" in ('ADMIN', 'EMPLOYER', 'SUPPLIER', 'CUSTOMER')) not null);
...
create table "transaction" ("id" serial primary key, "user_id" int4 not null, "delay_id" int4 null, "order_id" int4 null, "tool_id" int4 null, "opex_id" int4 null, "nominal" numeric(17, 4) not null, "code" varchar(255) not null, "type" text check ("type" in ('CREDIT', 'DEBIT')) not null, "mode" text check ("mode" in ('OFFLINE', 'CASH', 'ON_DELIVERY', 'CHEQUE_DRAFT', 'WIRED', 'ONLINE')) not null, "status" text check ("status" in ('NEW', 'CANCELLED', 'FAILED', 'PENDING', 'DECLINED', 'REJECTED', 'SUCCESS')) not null, "created_at" timestamptz(0) not null, "updated_at" timestamptz(0) null, "content" text null);
alter table "transaction" add constraint "transaction_order_id_unique" unique ("order_id");
alter table "transaction" add constraint "transaction_tool_id_unique" unique ("tool_id");
...
create table "tool" ("id" serial primary key, "name" varchar(255) not null, "transaction_id" int4 null, "created_at" timestamptz(0) not null, "updated_at" timestamptz(0) null);
alter table "tool" add constraint "tool_transaction_id_unique" unique ("transaction_id");
...
alter table "transaction" add constraint "transaction_tool_id_foreign" foreign key ("tool_id") references "tool" ("id") on update cascade on delete cascade;
...
alter table "tool" add constraint "tool_transaction_id_foreign" foreign key ("transaction_id") references "transaction" ("id") on update cascade on delete cascade;
