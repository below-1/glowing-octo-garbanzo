...
export default async (fastify: FastifyInstance) => {
  ...
  fastify.post<{ Body: CreatePayload, Params: ID }>('/:id/payments', {
    handler: async (request, reply) => {
      ...
      const pay = new BigNumber(payload.nominal)
      ...
      const next_total = total_paid.plus(pay)
      if ( next_total.gt(total_paid) ) {
        reply.status(500).send({
          message: `total payment is greater than what should be paid. \
(next_total=${next_total.toFixed(4)}) > (total_paid=${total_paid.toFixed(4)})`
        })
        return
      }
