...
const SQL_sale_report_month = readFileSync(
  join(process.cwd(), 'sql/purchase_report_month.sql')
).toString()
...
async function gen_purchase_report_month (year: number, month: number) {
  ...
  const filename = `purchase_report_month_${year}_${padded_month}.xlsx`
  print({
    data: render_data,
    template: 'purchase_report_month.xlsx',
    destination: filename
  })
  return filename
}

gen_purchase_report_month(workerData.year, workerData.month)
  .then(result => {
